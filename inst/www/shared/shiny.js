    ...
    function main() {
      ...
      (function() {
        ...
        this.createSocket = function() {
          var self2 = this;
          var createSocketFunc = WindowShiny.createSocket || function() {
            var protocol = "ws:";
        ...
        addMessageHandler("inputMessages", function(message) {
          for (var i = 0; i < message.length; i++) {
            var $obj = $(".shiny-bound-input#" + import_utils2.$escape(message[i].id));
        ...
        addMessageHandler("shiny-insert-ui", function(message) {
          ...
          if (targets.length === 0) {
            ...
            WindowShiny.renderHtml(message.content.html, $([]), message.content.deps);
          } else {
        ...
        function getTabset(id) {
          var $tabset = $("#" + import_utils2.$escape(id));
          if ($tabset.length === 0)
        ...
        function getTabContent($tabset) {
          ...
          var $tabContent = $("div.tab-content[data-tabsetid='" + import_utils2.$escape(tabsetId) + "']");
          return $tabContent;
        }
        ...
        addMessageHandler("shiny-insert-tab", function(message) {
          ...
          WindowShiny.renderContent($tabContent[0], {
            html: "",
          ...
          function getDropdown() {
            if (message.menuName !== null) {
              var $dropdownATag = $("a.dropdown-toggle[data-value='" + import_utils2.$escape(message.menuName) + "']");
      ...
      WindowShiny.notifications = function() {
        ...
        function show() {
          ...
          WindowShiny.renderContent($content, {
            html: newHtml,
            deps: deps
          });
      ...
      WindowShiny.modal = {
        show: function show() {
          ...
          WindowShiny.renderContent($modal, {
            html: html,
            deps: deps
          });
        },
      ...
      var inputBindings = WindowShiny.inputBindings = new BindingRegistry();
      var outputBindings = WindowShiny.outputBindings = new BindingRegistry();
      var OutputBinding = WindowShiny.OutputBinding = function() {
      };
      (function() {
        ...
        this.renderError = function(el, err) {
          ...
          if (err.type !== null) {
            errClass = errClass + " " + $.map(import_utils2.asArray(err.type), function(type) {
      ...
      var renderDependencies = WindowShiny.renderDependencies = function(dependencies) {
        if (dependencies) {
      ...
      WindowShiny.renderContent = function(el, content) {
      ...
      WindowShiny.renderHtml = function(html, el, dependencies) {
      ...
      function renderDependency(dep) {
        ...
        var $head = $("head").first();
        if (dep.meta && !restyle) {
          var metas = $.map(import_utils2.asArray(dep.meta), function(obj, idx) {
        ...
        if (dep.stylesheet) {
          var links = $.map(import_utils2.asArray(dep.stylesheet), function(stylesheet) {
          ...
          if (!restyle) {
            ...
            var bindDebouncer = new Debouncer(null, WindowShiny.bindAll, 100);
            setTimeout(function() {
        ...
        if (dep.script && !restyle) {
          var scripts = $.map(import_utils2.asArray(dep.script), function(scriptName) {
      ...
      var InputBinding = WindowShiny.InputBinding = function() {
      };
      (function() {
        this.find = function(scope) {
          throw "Not implemented";
      ...
      $.extend(textInputBinding, {
        ...
        _getLabelNode: function _getLabelNode(el) {
          return $(el).parent().find('label[for="' + import_utils2.$escape(el.id) + '"]');
        }
      });
      ...
      $(document).on("click", ".slider-animate-button", function(evt) {
        evt.preventDefault();
        var self2 = $(this);
        var target = $("#" + import_utils2.$escape(self2.attr("data-target-id")));
        var startLabel = "Play";
        var stopLabel = "Pause";
      ...
      $.extend(dateInputBinding, {
        ...
        _getLabelNode: function _getLabelNode(el) {
          return $(el).find('label[for="' + import_utils2.$escape(el.id) + '"]');
        },
          ...
          if (typeof config.data("nonempty") !== "undefined") {
            ...
            options = $.extend(options, {
              onItemRemove: function onItemRemove(value) {
                if (this.getValue() === "")
                  $("select#" + import_utils2.$escape(el.id)).empty().append($("<option/>", {
                    value: value,
                    selected: true
                  })).trigger("change");
              },
              onDropdownClose: function onDropdownClose($dropdown) {
                if (this.getValue() === "")
                  this.setValue($("select#" + import_utils2.$escape(el.id)).val());
              }
            });
          } else {
            el.nonempty = false;
      ...
      $.extend(radioInputBinding, {
        ...
        getValue: function getValue(el) {
          var checked_items = $('input:radio[name="' + import_utils2.$escape(el.id) + '"]:checked');
        ...
        setValue: function setValue(el, value) {
          if ($.isArray(value) && value.length === 0) {
            $('input:radio[name="' + import_utils2.$escape(el.id) + '"]').prop("checked", false);
          } else {
            $('input:radio[name="' + import_utils2.$escape(el.id) + '"][value="' + import_utils2.$escape(value) + '"]').prop("checked", true);
          }
        },
        getState: function getState(el) {
          var $objs = $('input:radio[name="' + import_utils2.$escape(el.id) + '"]');
      ...
      $.extend(checkboxGroupInputBinding, {
        ...
        getValue: function getValue(el) {
          var $objs = $('input:checkbox[name="' + import_utils2.$escape(el.id) + '"]:checked');
        ...
        setValue: function setValue(el, value) {
          $('input:checkbox[name="' + import_utils2.$escape(el.id) + '"]').prop("checked", false);
          if (value instanceof Array) {
            for (var i = 0; i < value.length; i++) {
              $('input:checkbox[name="' + import_utils2.$escape(el.id) + '"][value="' + import_utils2.$escape(value[i]) + '"]').prop("checked", true);
            }
          } else {
            $('input:checkbox[name="' + import_utils2.$escape(el.id) + '"][value="' + import_utils2.$escape(value) + '"]').prop("checked", true);
          }
        },
        getState: function getState(el) {
          var $objs = $('input:checkbox[name="' + import_utils2.$escape(el.id) + '"]');
          var options = new Array($objs.length);
          for (var i = 0; i < options.length; i++) {
            options[i] = {
              value: $objs[i].value,
        ...
        setValue: function setValue(el, value) {
          ...
          if (value) {
            ...
            anchors.each(function() {
              if (self2._getTabName($(this)) === value) {
                $(this).tab("show");
                success = true;
                return false;
              }
      ...
      (function() {
        ...
        this.$container = function() {
          return $("#" + import_utils2.$escape(this.id) + "_progress.shiny-file-input-progress");
        };
        this.$bar = function() {
          return $("#" + import_utils2.$escape(this.id) + "_progress.shiny-file-input-progress .progress-bar");
        };
      ...
      $.extend(fileInputBinding, {
        ...
        _enableDocumentEvents: function _enableDocumentEvents() {
          var _this2 = this;
          var $doc = $("html"), _this$_ZoneClass = this._ZoneClass, ACTIVE = _this$_ZoneClass.ACTIVE, OVER = _this$_ZoneClass.OVER;
        ...
        _handleDrop: function _handleDrop(e, el) {
          var files = e.originalEvent.dataTransfer.files, $el = $(el);
        ...
        unsubscribe: function unsubscribe(el) {
          var $el = $(el), $zone = this._zoneOf(el);
      ...
      function initShiny() {
        ...
        function unbindInputs() {
          var scope = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : document;
          var includeSelf = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : false;
          var inputs2 = $(scope).find(".shiny-bound-input");
      ...
      $(document).on("keydown", function(e) {
        if (e.which !== 114 || !e.ctrlKey && !e.metaKey || e.shiftKey || e.altKey)
          return;
        var url = "reactlog?w=" + window.escape(WindowShiny.shinyapp.config.workerId) + "&s=" + window.escape(WindowShiny.shinyapp.config.sessionId);
        window.open(url);
        e.preventDefault();
      });
      $(document).on("keydown", function(e) {
        ...
        var url = "reactlog/mark?w=" + window.escape(WindowShiny.shinyapp.config.workerId) + "&s=" + window.escape(WindowShiny.shinyapp.config.sessionId);
