...
#' @param clickClip Should clicking events be constrained to the plotting area?
...
#' @param hoverDelay Deprecated; use hoverOpts insetead. Also see the
#'   \code{\link{hoverOptions}} function.
#' @param hoverDelayType Deprecated; use hoverOpts insetead. Also see the
#'   \code{\link{hoverOptions}} function.
#' @param hoverOpts An object representing hovering options. This should be
#'   generated by the \code{\link{hoverOptions}} function.
#' @param brushId If not \code{NULL}, the plot will allow the user to "brush" in
...
#' @param brushOpts An object representing brushing options. This should be
...
imageOutput <- function(outputId, width = "100%", height="400px",
                        ...
                        hoverId = NULL, hoverDelay = NULL, hoverDelayType = NULL,
                        hoverOpts = hoverOptions(),
                        brushId = NULL, brushOpts = brushOptions(),
                        inline = FALSE) {
  ...
  if (!is.null(hoverDelay) || !is.null(hoverDelayType)) {
    ...
    hoverOpts <- hoverOptions(delay = hoverDelay, delayType = hoverDelayType)
  }
  ...
  if (!is.null(hoverId)) {
    args <- c(args, list(
      ...
      `data-hover-clip` = hoverOpts$clip,
      `data-hover-delay` = hoverOpts$delay,
      `data-hover-delay-type` = hoverOpts$delayType
    ))
  }
  ...
  if (!is.null(brushId)) {
    args <- c(args, list(
      ...
      `data-brush-clip` = brushOpts$clip,
      `data-brush-color` = brushOpts$color,
      `data-brush-outline` = brushOpts$outline,
      `data-brush-opacity` = brushOpts$opacity,
      `data-brush-delay` = brushOpts$delay,
      `data-brush-delay-type` = brushOpts$delayType,
      `data-brush-direction` = brushOpts$direction
    ))
  }
...
plotOutput <- function(outputId, width = "100%", height="400px",
  ...
  res <- imageOutput(outputId, width, height, clickId, clickClip, hoverId,
                     hoverDelay, hoverDelayType, hoverOpts, brushId, brushOpts,
                     inline)
