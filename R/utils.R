...
# Create a zero-arg function from either:
# * quoted expression and environment
# * A rlang::quo()sure
# @examples
# makeFunction(quote(print(3)))
makeFunction <- function(x, env = parent.frame()) {
  func <- as_function(as_quosure(x, env = env))
  # as_function returns a function that takes `...`.
  # We want one that takes no args.
  formals(func) <- pairlist()
  func
}

#' @describeIn createRenderFunction Convert a user expression into a debug-able
#'   function that can then be passed to [createRenderFunction()]. This function
#'   is designed to be called directly inside the 'top-level' `environment()` of
#'   a custom render function (see the examples below).
#' @param expr A user-supplied reactive expression (or quosure).
#' @param env The desired evaluation environment for the function.
...
exprToFunction <- function(expr, env = parent.frame(), quoted = FALSE,
  ...
  wrapFunctionLabel(
    makeFunction(expr, env = env),
    label, ..stacktraceon = ..stacktraceon
  )
}
...
#'
#' Superceded by [exprToFunction()]: use that function instead for a more
#' intuitive way to pass reactive expressions to [createRenderFunction()].
#'
...
#' @param wrappedWithLabel,..stacktraceon Advanced use only. For stack
#'   manipulation purposes; see [stacktrace()].
#' @export
#' @keywords internal
installExprFunction <- function(expr, name, eval.env = parent.frame(2),
  ...
  func <- exprToFunction(
    expr, env = eval.env, quoted = quoted,
    label = label, ..stacktraceon = ..stacktraceon
  )
...
#'
#' Superceded by [exprToFunction()]: use that function instead for a simpler
#' and more robust implementation of custom render functions.
#'
#' @param q A quosure.
...
quoToFunction <- function(q, label, ..stacktraceon = FALSE) {
  exprToFunction(q, label = label, ..stacktraceon = ..stacktraceon)
}
