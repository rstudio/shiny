...
test_that("integration tests", {
  ...
  expect_equal(df$num, c(57L, 56L, 55L, 54L, 38L, 37L, 36L, 35L,
    34L, 33L, 32L, 31L, 30L))
  expect_equal(df$call, c("A", "B", "reactive", "<reactive:C>", "C", "renderTable",
    "func", "force", "withVisible", "withCallingHandlers", "domain$wrapSync",
    "promises::with_promise_domain", "captureStackTraces"))
  expect_equal(nzchar(df$loc), c(TRUE, TRUE, TRUE, FALSE, FALSE, TRUE,
    FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE))

  df <- causeError(full = TRUE)
  # dumpTests(df)

  expect_equal(df$num, c(60L, 59L, 58L, 57L, 56L, 55L, 54L, 53L,
  ...
  expect_equal(df$call, c("h", ".handleSimpleError", "stop",
    "A", "B", "reactive", "<reactive:C>", "..stacktraceon..", ".func", "withVisible",
